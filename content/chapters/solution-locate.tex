...
When used on a setup of $N$ cameras with framerate $f$ each, the Locate step would receive $N{\cdot}f$ independent frames each second.
...
Ideally, all tracers should be detected, with precision, recall and F1-score equal to 1.
If however there are mistakes in the locating process, the errors would propagate to future steps:
\begin{itemize}
	\itemsep 0em
...
\end{itemize}
As such, it is better to prioritize false negatives over false positives, which means prioritize precision over recall.
