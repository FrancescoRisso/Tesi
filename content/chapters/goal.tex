...
From all these considerations, the bubbles were created with a coating made of a proprietary substance~\cite{bubble-substance} created by Sage Action, filled with helium.
...
This task was tackled by their internal team of embedded developers, that built a global shutter camera, with resolution 960{$\times$}960 and 16 bits per pixel, that could operate at a framerate of 24 FPS (or 30 FPS, if the resolution was lower).
Each camera was equipped with an FPGA module, that was programmed to perform a background subtraction and a binarization, providing in output binary images, with white bubbles over a black background.
